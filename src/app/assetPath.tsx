...
export const fetchAssetPath = async (itemId: string, client?: SupabaseClient): Promise<Crumb[]> => {
  ...
  // place (station / structure / solar system), or null when the caller may
  // not see the place.
  ...
  if (!outermost) return [ASSETS_CRUMB]
  ...
  const typeNames = await fetchTypeNames(map((a: AncestorRow) => Number(a.type_id), ancestors))
  const ancestorCrumbs = map(
    (a: AncestorRow) => ({
  ...
  // No place on the outermost row: the caller may see the item but not where
  // it is — a share recipient, whose walk ends at the shared item because a
  // root item's place is owner-only (character_asset_location). The trail
  // inside the shared item still shows; where it sits does not.
  if (outermost.location_id == null) return [ASSETS_CRUMB, ...ancestorCrumbs]

  const root = { id: String(outermost.location_id), type: outermost.location_type }
  const resolved = await resolveLocations([root], supabase)
