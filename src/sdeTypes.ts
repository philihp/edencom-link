...
// and src/jobs/sdeMirror.js). Replaces the build-time generated JSON: data now
// refreshes nightly without a rebuild. By-id lookups are cached per server
// process for 6h (misses never cached — see src/sdeCache.ts); searches hit the
// RPC uncached (user-action paths, not per-row render paths).
