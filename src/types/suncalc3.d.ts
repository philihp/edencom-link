/*
 * Type definitions for `suncalc3`.
 *
 * Vendored verbatim from the library's own declaration file
 * (https://github.com/hypnos3/suncalc3/blob/master/suncalc.d.ts), wrapped in a
 * `declare module` so it applies to the package import. suncalc3@2.0.5 names
 * this file in its package.json `types` field but does not actually publish it,
 * and there is no @types/suncalc3 — so we ship the upstream copy here. Replace
 * with the package's own types if a future release starts shipping them.
 */
/* eslint-disable no-var -- vendored verbatim; upstream declares `export var`. */
