// character-mercenary-dens as a Vercel Workflow — phase 3, PR 5 of the cron →
// Workflows migration (docs/cron-to-workflows/03-per-character.md). Copies the
// fan-out shape the earlier per-character jobs established; the only differences
// are the scope, the job label, and the job module it loads.
//
// This job does more per character than the others — an SCD-2 identity reconcile
// plus an append-only status observation plus one extra per-den detail ESI call —
// but all of that stays inside the untouched job module. The workflow only
// replaces the dispatch layer, exactly as for the simpler jobs; a per-character
// step just gets a longer budget than a queue message did, which suits the extra
// per-den calls.
//
...
// — token refresh, the per-character heartbeat pair, the per-den detail calls,
// the SCD-2 reconcile and the status observation insert — stays inside the
// untouched job module (runCharacterMercenaryDens → forEachCharacter in
// src/jobs/lib.js).
//
...
const SCOPES = ['esi-structures.read_character.v1']
const LANES = 4
...
async function syncCharacter(registrationId: string, taskId?: string) {
  ...
  await withRefreshTask(taskId, () => runCharacterMercenaryDens({ registrationIds: [registrationId] }))
}

export async function characterMercenaryDensWorkflow(target?: OnDemandTarget) {
  'use workflow'
