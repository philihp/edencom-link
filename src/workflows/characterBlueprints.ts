...
//
// This is the first *paginated* SCD-2 reconciler to migrate (blueprints come
// back over multiple ESI pages, drained by the untouched job module before the
// reconcile). The per-character step just gets a longer budget than a queue
// message did, which suits the paging; the reconcile itself stays inside the job
// module (runCharacterBlueprints → forEachCharacter in src/jobs/lib.js).
//
...
// — token refresh, the per-character heartbeat pair, the ESI paging and the
// SCD-2 reconcile — stays inside the untouched job module.
//
