...
// registrationId is the registration
...
async function syncCharacter(registrationId: string, taskId?: string) {
  ...
  await withRefreshTask(taskId, () => runCharacterStatus({ registrationIds: [registrationId] }))
}

export async function characterStatusWorkflow(target?: OnDemandTarget) {
  'use workflow'
