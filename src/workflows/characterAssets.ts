// character-assets as a Vercel Workflow — phase 3, PR 7 (the last) of the cron →
...
//
// This is the biggest-volume, most user-visible per-character job — paginated
// assets plus the player-assigned name resolution (/assets/names/) — so it goes
// last. All of that (ESI paging, the name pull, the SCD-2 reconcile) stays
// inside the untouched job module (runCharacterAssets → forEachCharacter in
// src/jobs/lib.js); the per-character step just gets a longer budget than a
// queue message did, which suits the paging.
//
...
// — token refresh, the per-character heartbeat pair, the paging and the SCD-2
// reconcile — stays inside the untouched job module.
//
