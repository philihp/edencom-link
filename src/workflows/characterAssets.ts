// character-assets as a Vercel Workflow — phase 3, PR 7 (the last one) of the
// cron → Workflows migration (docs/cron-to-workflows/03-per-character.md).
// Deliberately saved for last: the biggest volume and the most user-visible
// table. Copies the fan-out shape the earlier per-character jobs established;
// the only differences are the scope, the job label, and the job module it
// loads.
//
// The multi-page ESI pull, the per-item asset-name resolution, and the SCD-2
// reconcile all stay inside the untouched job module — the workflow only
// replaces the dispatch layer, and a per-character step gets a longer budget
// than a queue message did, which suits the largest hangars.
//
...
// asset names, the SCD-2 reconcile — stays inside the untouched job module
// (runCharacterAssets → forEachCharacter in src/jobs/lib.js).
//
...
async function syncCharacter(registrationId: string) {
  ...
  await runCharacterAssets({ registrationIds: [registrationId] })
}

export async function characterAssetsWorkflow(onDemand?: OnDemand) {
  'use workflow'
