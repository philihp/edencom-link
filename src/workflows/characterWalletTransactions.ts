...
// — which skips enumeration. The job module is CLI-runnable too. One job, two triggers, one
// run*().
...
async function syncCharacter(registrationId: string, taskId?: string) {
  ...
  await withRefreshTask(taskId, () => runCharacterWalletTransactions({ registrationIds: [registrationId] }))
}

export async function characterWalletTransactionsWorkflow(target?: OnDemandTarget) {
  'use workflow'
