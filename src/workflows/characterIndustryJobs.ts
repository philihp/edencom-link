// character-industry-jobs as a Vercel Workflow — phase 3, PR 3 of the cron →
// Workflows migration (docs/cron-to-workflows/03-per-character.md). Copies the
// fan-out shape characterWalletTransactions.ts / characterOrders.ts established;
// the only differences are the scope, the job label, and the job module it loads.
//
...
// (runCharacterIndustryJobs → forEachCharacter in src/jobs/lib.js).
//
...
const SCOPES = ['esi-industry.read_character_jobs.v1']
const LANES = 4
...
async function syncCharacter(registrationId: string, taskId?: string) {
  ...
  await withRefreshTask(taskId, () => runCharacterIndustryJobs({ registrationIds: [registrationId] }))
}

export async function characterIndustryJobsWorkflow(target?: OnDemandTarget) {
  'use workflow'
      ...
      (p, id) =>
        p.then(() =>
          syncCharacter(id, target?.taskId).catch((err) => {
            console.error(`[character-industry-jobs] character ${id} failed:`, err)
            failures.push(id)
          })
        ),
      Promise.resolve(),
  ...
  if (failures.length > 0) {
    throw new AggregateError(
      ...
      `character-industry-jobs: ${failures.length} character step(s) failed`
    )
  }
}
