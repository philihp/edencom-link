...
| [02-sql-name-joins.md](02-sql-name-joins.md) | medium | JOIN SDE names inside the Postgres functions (CSV exports, asset search) | ✅ **Done** — the 7 CSV/Sheets functions gained appended `type_name` (and `blueprint_type_name`/`product_type_name` for industry jobs); `character_asset_search`/`corp_asset_search` gained `type_name`/`root_location_name`/`system_id`; `asset_ancestors` gained `type_name`; and the two `*_asset_location_summary` rollups gained `station_name`/`system_id`. All additive, all LEFT JOINs over `sde_published_type`/`sde_station`. Consumer simplification (dropping `fetchTypeNames`, slimming `resolveLocations`) left as follow-ups. |
