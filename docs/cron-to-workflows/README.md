...
| [04-per-corporation.md](04-per-corporation.md) | `corp-wallet-transactions` ✅, `corp-industry-jobs` ✅, `corp-assets` | per-corp queue fan-out → fan-out workflow | 🚧 In progress — through `corp-industry-jobs` (first reconciler); `corp-assets` remains |
