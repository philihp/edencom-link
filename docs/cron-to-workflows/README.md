...
| `fanOutPerCorporationCronJob` (1 msg/corp) | `corp-assets`, `corp-industry-jobs`, ~~`corp-wallet-transactions`~~ (migrated in phase 4) | Queue consumer runs per corp |
...
| [04-per-corporation.md](04-per-corporation.md) | `corp-wallet-transactions` ✅, `corp-industry-jobs`, `corp-assets` | per-corp queue fan-out → fan-out workflow | 🚧 In progress — `corp-wallet-transactions` migrated (establishes the per-corp fan-out shape); `corp-industry-jobs`, `corp-assets` remain |
