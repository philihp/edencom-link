...
> **✅ Complete, end to end.** Every extract job — scheduled _and_
> on-demand — executes as a `start()`ed workflow under `src/workflows/`.
> The queue consumer (`/api/queue/jobs`), its `jobs` topic, the queue
> fan-out helpers in `src/utils/cron.ts`, and the `character-implants`
> pilot are all deleted (phase 5, [05-contract.md](05-contract.md)); the
> queue infrastructure survives only for the separate `innominate`
> appraisal topic, and `runDirectCronJob` only for the unscheduled
> `esf-data`/`sheet-csv` bootstrap routes. The doc below is the plan as
> executed, kept for the reasoning.
...
| [05-contract.md](05-contract.md)               | —                                                                                                                                                                                               | retire dead cron helpers, decide the on-demand queue path, retire the `character-implants` pilot | ✅ **Done** — on-demand chose (b): `dispatchRefresh` `start()`s the same workflows; consumer + `jobs` topic + queue fan-out helpers deleted; pilot retired |
