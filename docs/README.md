...
| [front-page-lore.md](front-page-lore.md) | Recast the front-page copy from ERP-vendor parody into the EDENCOM reading: one shared record, the way the institution ran one threat picture | Shipped. Records the voice rules, and the three stats-band numbers that drift out of true when `vercel.json` changes |
...
| [jobs-page.md](jobs-page.md) | `/jobs`: one page for job freshness, live activity, next scheduled run; replaces `/character/refresh` | Shipped — the page renders live heartbeats, open runs and refresh tasks; `/character/refresh` is deleted and permanently redirected |
