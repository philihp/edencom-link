...
- `pnpm run sde:build` — downloads CCP's SDE type/group, solar-system, NPC-station, blueprint industry, and planet data and writes `src/generated/sdeTypes.json` + `src/generated/sdeSystems.json` + `src/generated/sdeStations.json` + `src/generated/sdeBlueprints.json` + `src/generated/sdePlanets.json` (gitignored). Runs automatically as a `predev`/`prebuild` step; skips re-downloading any file that already exists (pass `--force` to refresh). See `src/buildSde.js`.
- `pnpm run esf:build` — builds the 6 protobuf data files `@eveshipfit/react`'s `EveDataProvider` reads (`public/esf-data/*.pb2`, gitignored) from CCP's official SDE, per `src/esf.proto`. Also a `predev`/`prebuild` step; skips if the files exist (`--force` to refresh). See `src/buildEsfData.js`.
