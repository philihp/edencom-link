...
- **MCP server:** `/api/mcp` (Streamable HTTP, via `mcp-handler`; route at `src/app/api/mcp/route.ts`) exposes read-only tools over the extracted data, registered from three modules sharing the plumbing in `lib.ts`:
  ...
  `list_market_orders` and `list_industry_jobs` take an optional `as_of` (parsed by the same `parseAtParam` the Sheets `at=` uses) that reconstructs a past snapshot through the SCD-2 functions `character_orders()` / `character_industry_jobs()` / `corp_industry_jobs()` instead of the `is_current` views — those were `service_role`-only (they were written for the Sheets endpoints, which query as service role) until `20260725010000_snapshot_rpc_authenticated_grants.sql` granted execute to `authenticated`; they're SECURITY INVOKER, so RLS still scopes rows.
