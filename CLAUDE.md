...
- **The build downloads nothing from CCP.** SDE type/system/station/blueprint/planet lookups read the nightly-mirrored `sde_*` tables at runtime via the async loaders in `src/sde*.ts` (SDE-to-database cutover; the old `sde:build`/`src/buildSde.js` build-time JSON generator was removed). The one remaining build step is `esf:build`, which also reads the mirror:
- `pnpm run esf:build` — builds the 6 protobuf data files `@eveshipfit/react`'s `EveDataProvider` reads (`public/esf-data/*.pb2`, gitignored) from the nightly-mirrored `sde_*` tables (public-read via the anon key; needs `NEXT_PUBLIC_SUPABASE_URL`/`_ANON_KEY`), per `src/esf.proto`. The only `predev`/`prebuild` step; skips if the files exist (`--force` to refresh). See `src/buildEsfData.js`.
...
### `src/buildEsfData.js` — ship-fitting protobuf generator (run via `pnpm run esf:build`)
- Encodes the 6 `public/esf-data/*.pb2` files `@eveshipfit/react`'s `EveDataProvider` reads client-side, per `src/esf.proto`. Reads its SDE inputs (types/groups/categories/marketGroups/dogmaAttributes/dogmaEffects/typeDogma) from the nightly-mirrored `sde_*` tables via the public-read anon key — no CCP download, no `unzip` binary — pages each `sde_<stem>` table 1000 rows at a time and consumes each row's `data` jsonb (the raw SDE object). Then applies the vendored eveship.fit dogma patches (`src/esfPatches.json`) and encodes. Runs as the sole `predev`/`prebuild` step; skips if the `.pb2` files already exist (`--force` to regenerate). Needs `NEXT_PUBLIC_SUPABASE_URL`/`_ANON_KEY` (present in local `.env` and the Vercel build env). Exports `encodeEsfData()` (reads the mirror + patches, returns `{ [fileName]: Buffer }` without touching disk) so `src/jobs/esfData.js` (`pnpm run esf-data`, `runEsfData({ build })`) can reuse the same encode to base64-upsert the 6 files into the `esf_data` table — the `sde-mirror` workflow's `encodeEsf` step runs it after each ingest so the ship-fit data refreshes without a redeploy (Phase 1 of `docs/sde-db-cutover/04-esf-workflow.md`; Phase 2 serves those rows at `/esf/[file]`, Phase 3 retires the build step). The module only self-runs `run()` (the build step) when invoked as a CLI.
