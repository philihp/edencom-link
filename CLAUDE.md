...
| `mercenary_den_share_opt_out` | The exceptions to alliance den sharing, which is **on by default**: one row = "this user does not share their dens or enemy-den intel." Audience is derived, not stored — the `mercenary_den_visible_registrations()` SECURITY DEFINER helper returns own + same-corp + same-alliance registrations minus opted-out owners, and drives the RLS policies on `character_mercenary_den_over_time` and `mercenary_den_enemy_intel`. Replaced the per-corporation opt-in table `character_mercenary_den_share`. RLS: a user reads/inserts/deletes only their own row | `user_id`, `created_at` |
