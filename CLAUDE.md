...
- SDE type/system/station/blueprint/planet lookups are **no longer** downloaded at build time — they read the nightly-mirrored `sde_*` tables at runtime via the async loaders in `src/sde*.ts` (see the Data sources section). The old `sde:build` step and `src/buildSde.js` were removed in the SDE-to-database cutover.
- `pnpm run esf:build` — builds the 6 protobuf data files `@eveshipfit/react`'s `EveDataProvider` reads (`public/esf-data/*.pb2`, gitignored) from CCP's official SDE, per `src/esf.proto`. Runs as the `predev`/`prebuild` step (now the only one); skips if the files exist (`--force` to refresh). See `src/buildEsfData.js`.
...
- Type/group/category (and system/station/blueprint/planet) lookups are resolved at runtime from the nightly-mirrored `sde_*` tables/views (`sde_published_type`, `sde_kspace_system`, `sde_station`, `sde_planet`, `sde_blueprint_product`, and the `sde_search_type`/`sde_search_system` RPCs — see `supabase/migrations/20260716010000_sde_mirror.sql`), populated by the `sde-mirror` Vercel Workflow from CCP's official Static Data Export (`developers.eveonline.com/static-data`, JSONL format, refreshed each game patch — never Fuzzwork's third-party mirror, which is off-limits for this project). The async loaders in `src/sde*.ts` (`src/sdeTypes.ts` exposes `getSdeType`/`getSdeTypes`/`getSdeTypeNames`/`searchSdeTypes`/`searchSdeTypesAll`, used by `src/app/typeNames.ts`, `src/app/blueprint/api.ts`, and `src/app/api/type/search/route.ts`) query these tables through the public-read anon client `src/utils/supabase/sde.ts`, caching by-id lookups per server process for 6h (misses never cached; shared cache in `src/sdeCache.ts`). If a needed field isn't in a view, extend the view in `src/jobs/sdeMirror.js` + a migration.
...
### SDE loaders (`src/sde*.ts`) — DB-backed over the nightly mirror
All data functions are **async** — they query the `sde_*` tables/views/RPCs through the anon client `src/utils/supabase/sde.ts`. By-id lookups cache per server process for 6h (misses never cached) via the shared `src/sdeCache.ts` (`createByIdCache` + `bulkLookup`, which chunks `.in()` queries at 500 ids); searches and blueprint lookups are uncached. A DB hiccup logs and returns empty/null so a page degrades to `#id` fallbacks rather than throwing.

### `src/sdeCache.ts`
- `createByIdCache<T>()` — per-process 6h-TTL by-id cache (found rows only)
- `bulkLookup(cache, ids, fetchMissing)` — serve from cache, fetch the rest in 500-id chunks; `fetchMissing` owns its own error handling (log + return `[]`)

### `src/sdeTypes.ts` (async; over `sde_published_type` + `sde_search_type`)
- `getSdeTypes(typeIDs[])` — bulk id→`{ typeID, name, groupID, categoryID }`, cached
- `getSdeType(typeID)` — single lookup (via `getSdeTypes`), or `null`
- `getSdeTypeNames(typeIDs[])` — bulk id→name lookup
- `searchSdeTypesAll(query)` — case-insensitive substring search, ranked by match coverage, capped at 1000 rows (RPC max — every "too many types" guard triggers far below that). `SdeSearchResult` now carries `categoryID`, so callers filtering by category no longer need a per-row `getSdeType`
- `searchSdeTypes(query, limit?)` — same search capped to `limit` results, for autocomplete UIs

### `src/sdeSystems.ts` (async; over `sde_kspace_system` + `sde_search_system`)
- `getSdeSystems(systemIDs[])` — bulk id→`{ systemID, name, security }`, cached
...
### `src/sdeStations.ts` (async; over `sde_station`)
- `getSdeStations(stationIDs[])` — bulk id→`{ stationID, name, systemID }`, cached
...
### `src/sdePlanets.ts` (async; over `sde_planet`, which carries `system_name`)
- `getSdePlanets(planetIDs[])` — bulk id→`{ planetID, systemID, systemName, celestialIndex, typeID, roman, name }`, cached. `name` is derived as `"<system> <roman(celestialIndex)>"` (e.g. `RXA-W1 III`) — the same key the hand-maintained mercenary-den intel uses. No longer depends on `sdeSystems` (the view provides the system name)
...
### `src/sdeBlueprints.ts` (async; over `sde_blueprint_product`)
...
### `src/flags.ts`
- `corpsesFlag` — per-user Vercel Flag (`flags/next`) backed by the `user_settings.flags` array; the account owner's opt-in gate for their public `/corpses/[characterID]` share page (false for signed-out users and anyone without the flag)
...
| `sde_mirror_state` | One row per SDE build the ingest has seen (skip check) | `build_number` (PK), `started_at`, `completed_at` |
...
- **Per-user opt-in flags:** an entry in the `user_settings.flags` text array, exposed as a Vercel Flag in `src/flags.ts` (`corpsesFlag`). There is no UI to set flags (edit the DB row / use `/account/debug` to inspect). `/corpses/[characterID]` is a public share page gated by it: the owner enabling `corpses` is what makes their share page resolve instead of 404.
...
- **Name resolution:** `universe_name` table caches ESI `universeNames` lookups (kept fresh by the `universe-names` job). `resolveBatch()` handles bisect-on-error for large batches. Type names (items/ships) come from the nightly-mirrored SDE tables via the async `src/sdeTypes.ts` loader, not the extract DB.
