...
| `character_mercenary_den` | Live snapshot of a character's deployed Mercenary Dens + status (reconciled like `character_order`) | `character_id`, `den_id`, `planet_id`, `type_id`, `state`, `development_level`/`_amount`, `anarchy_level`/`_amount`, `infomorphs`, `reinforcement_end`, `skyhook_id`/`_corporation_id`, `recorded_at` |
