...
| `mercenary_den_share_opt_out` | The exceptions to alliance den sharing, which is **on by default**: one row = "this character's dens are not shared." Audience is derived, not stored — `mercenary_den_shared_with_caller(reg_id)` joins the world-readable `character_directory` to `my_corporation_ids()`/`my_alliance_ids()` and subtracts opted-out registrations, driving the RLS policies on `character_mercenary_den_over_time` and `mercenary_den_enemy_intel`. All three helpers run on **invoker** rights (no SECURITY DEFINER) — resolving a den owner through `character_directory` rather than `registration` is what removed the need for definer bridges. Keyed by registration to match that join, and deliberately bare (no `created_at`, which would let anyone correlate a user's alts by opt-out timestamp); the UI writes all of a user's registrations at once and `/character/callback` carries the choice to newly linked characters. Replaced the per-corporation opt-in table `character_mercenary_den_share`. RLS: world-readable to authenticated, writes limited to the caller's own registrations | `registration_id` |
