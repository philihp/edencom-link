...
- **Per-corporation jobs** (`corp-assets`, `corp-industry-jobs`, `corp-wallet-transactions`): `fanOutPerCorporationCronJob` groups scoped characters by corporation (`groupCharacterIdsByCorporation` in `src/supabase.js`) and sends one queue message per corp, avoiding the same-corp concurrent-reconcile race described above. **Phase 4 of the cron → Workflows migration is under way** (`docs/cron-to-workflows/04-per-corporation.md`): the scheduled path moves to a per-corporation fan-out Vercel Workflow — the cron route `start()`s a like-named workflow that groups the characters by corp itself (the shared `enumerateCorporations` step in `src/workflows/lib.ts`, returning the same one-group-per-corp set plus singleton groups for unresolved characters) and runs one `'use step'` per corp across a couple of statically-assigned lanes. A corp's characters stay together in one step (never split — that would recreate the concurrent-reconcile race), and only *different* corps run concurrently; each failing corp is caught-and-continued with a summary rethrown so failures surface in Observability. Per-corp token refresh, role-fallback and heartbeats stay inside the untouched job module (`forEachCorporation`). Migrated so far (all `source: 'vercel-workflow'`): `corp-wallet-transactions` (`corpWalletTransactionsWorkflow`, the safe keyed-append leader) and `corp-industry-jobs` (`corpIndustryJobsWorkflow`, the first SCD-2 reconciler). The last reconciler (`corp-assets`) still uses `fanOutPerCorporationCronJob`. Only the scheduled trigger moves; the on-demand "Refresh ESI" queue path is untouched until phase 5.
