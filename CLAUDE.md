...
- **MCP server:** `/api/mcp` (Streamable HTTP, via `mcp-handler`; route at `src/app/api/mcp/route.ts`) exposes read-only tools over the extracted data, registered from three modules sharing the plumbing in `lib.ts`:
  ...
  The two blueprint tools read only the mirrored SDE data (`src/sdeBlueprints.ts`) — not the extract DB — and adjust the material bill through the `eve-industry` `cost()` modifiers exposed as tool params (`runs`, `material_efficiency`, `structure`, `rig`, `security`); passing a monitored `structure_id` instead derives the structure role bonus, ME rig and system-security multiplier from `corp_structure`/`corp_structure_rig` (RLS-scoped) and overrides those three params. **The rig is gated on applicability** (`src/app/blueprint/rigs.ts`, the same filter tables behind `/blueprint/[typeID]`): only a fitted ME rig whose filter covers the product's group/category can discount it, and the rigs skipped for that reason are reported. Because applicability is per product, the structure is loaded once per call and the bonus resolved per row — `blueprints_using_material` reports `rig_applied` on each row rather than naming one rig for the whole listing. `list_structures`' `me_bonus` has no product to test against, so it stays a best case and says so (`me_bonus_assumes_applicable_rig`).
