...
- **Unified share shape (Revision 3)** — `character_asset_share`, `character_fitting_share` and `lens` all carry the same audience columns: `corporation_ids bigint[]`, `alliance_ids bigint[]` (both `not null default '{}'`) and a nullable `secret`. Audiences **compose** — a row may pin corporations *and* carry a secret. `share_audience_matches(corporation_ids, alliance_ids, secret)` is the shared predicate (`asset_share_matches_caller` delegates to it): fully public is the row state `secret is null and corporation_ids = '{}' and alliance_ids = '{}'`, otherwise the arrays are intersected against `my_corporation_ids()`/`my_alliance_ids()`. The arrays pin **specific** ids, not "whoever is currently in my corp". RLS never sees a URL token — a link share's token is an HMAC of `secret ‖ TOKEN_SALT` over the share id, verified at the app layer (`src/shareToken.ts`), so a database dump alone can't mint links. Design in `docs/sharing-layer/01-asset-share-table.md`
