...
- `blueprint_search(type_ids, system_ids, character_ids, corporation_ids, kind_filter, min_me, min_te, group_mode, row_limit)` — the MCP `list_blueprints` query: unions `character_blueprint` + `corp_blueprint`, resolves each row's `location_id` to a system (via `sde_station` / `universe_structure`), applies every filter, optionally collapses stacks by type (`group_mode = 'type'`), and returns one json object whose totals cover the whole filtered set while only `row_limit` rows are itemized. Security invoker, so RLS scopes it like a direct select
