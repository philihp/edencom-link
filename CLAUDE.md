...
- **Stack:** Next.js 16 (App Router) + React 19 + TypeScript 6, ESM (`"type": "module"`). Supabase (Postgres) for storage. `ramda` for utilities; `ts-pattern` for branching that dispatches on a string-literal union, where `.exhaustive()` turns a newly added member into a build error instead of a silent fallthrough (see `formatBlueprintRows` / `classForGroupId`). Neither replaces an early-return guard chain or a range comparison — `freshnessLevel`, `securityMultiplier` and the mercenary-den `colorOf` priority ladder stay as plain ifs.
...
- `pnpm test` — `node --test "test/**/*.test.ts"`. Node's built-in runner over the repo's few pure-logic modules (no framework, no transpile step — Node strips the TypeScript types itself, which is why the tests import with an explicit `.ts` extension and `allowImportingTsExtensions` is on). Most of the codebase is I/O against Supabase/ESI and is still verified by `pnpm run build` + `pnpm run lint` rather than tests.
- `pnpm run test:sql` — `psql $DATABASE_URL -f test/sql/blueprint_search.sql`, assertions for the Postgres side of the MCP blueprint query. Point `DATABASE_URL` at a **throwaway** database: the script creates stand-in tables named after the real views (everything runs in one transaction and rolls back).
- No `typecheck` script (rely on `next build` / editor).
...
- `pnpm run sheet-csv` — encodes the industry-planning spreadsheet's static CSVs (`static-inputs-{twines,miros}.csv`, `static-outputs-{twines,miros}.csv`, `invention.csv`, `types-{twines,miros}.csv`) from the nightly-mirrored `sde_types`/`sde_blueprints` tables (public-read anon key) and upserts them as plain text into the `sheet_csv` table via the service role (`src/jobs/sheetCsv.js` → `encodeSheetCsv()` in `src/buildSheetCsv.js`; a JS port of `docs/sheet-csv/reference/full_sheet_gen.py`). Like `esf-data`, not separately scheduled: it runs as a tail step of the `sde-mirror` workflow on every non-skipped pass (`encodeSheets`, gated on `sde_types`+`sde_blueprints` draining); idempotent (a no-op when `sheet_csv` is already at the current build). Also runnable manually — the CLI, or the unscheduled `CRON_SECRET`-protected `/api/cron/sheet-csv` route (bootstrap / `?force=1`). The `/sheets/[file]` route serves the rows as `text/csv` for Google Sheets `=IMPORTDATA()` — public and CDN-cached (SDE-derived, no player data, no `api_token`), ETag keyed on `sde_build`.
...
| `/sheets/[file]` | `src/app/sheets/[file]/route.ts` |
...
Notable pages:
- `/mercenary-dens` — unions the DB's extracted dens (own + corp/alliance-shared via RLS) with hand-maintained intel in `src/app/mercenary-dens/data.ts` (`STAGING`, `LINKS` system adjacency, `TEMPERATE_PLANETS`, per-planet `den` ownership — the volatile, hand-edited part), rendered as a server-side SVG topology (`topology.tsx`, red reinforced > green ours > yellow external) plus a table; `shareAlliance.tsx` is the opt-in picker for which of your alliances to share dens & intel with (usually just the one — see `character_mercenary_den_share`).
...
- **Per-character jobs** (`character-wallet-transactions`, `character-orders`, `character-industry-jobs`, `character-status`, `character-mercenary-dens`, `character-blueprints`, `character-assets`): on the scheduled path these have all migrated to **per-character fan-out Vercel Workflows** (phase 3 of the cron → Workflows migration, `docs/cron-to-workflows/03-per-character.md`, now **complete**). Each cron route `start()`s a like-named workflow under `src/workflows/` (`characterWalletTransactionsWorkflow`, `characterOrdersWorkflow`, `characterIndustryJobsWorkflow`, `characterStatusWorkflow`, `characterMercenaryDensWorkflow`, `characterBlueprintsWorkflow`, `characterAssetsWorkflow`) that enumerates the scoped characters itself (the shared `enumerateCharacters` step in `src/workflows/lib.ts`, which unions the scopes it's given) and runs one `'use step'` per character across a few statically-assigned lanes — round-robin via `transpose(splitEvery(LANES, ids))`, so replay is deterministic — draining each lane sequentially with a `reduce` promise-chain. Each failing character is caught-and-continued (logged as it happens) and the collected ids are thrown together as an `AggregateError` once every lane drains, so the run surfaces failures in Observability instead of the queue's silent re-loop. Per-character token refresh + heartbeats stay inside the untouched job modules (`forEachCharacter`, or `forEachCharacterAnyScope` for `character-status`, which fronts six endpoints with different scopes), all `source: 'vercel-workflow'`. `fanOutPerCharacterCronJob`/`fanOutPerCharacterAnyScopeCronJob` are now unused on the scheduled path (deleted in phase 5). Only the scheduled trigger moved; the on-demand "Refresh ESI" queue path (`dispatchRefresh.ts` → `/api/queue/jobs`, whose consumer records its own per-character heartbeat with `source: 'vercel'`) is untouched until phase 5. `character-skills` (`characterSkillsWorkflow`) uses this same fan-out shape but has no `vercel.json` crons entry of its own — `character-status` covers skills on the schedule (calling `syncCharacterSkills` inline), so the `/api/cron/character-skills` route is a deliberately unscheduled manual/backfill trigger that `start()`s the workflow.
- **Per-corporation jobs** (`corp-assets`, `corp-industry-jobs`, `corp-wallet-transactions`): `fanOutPerCorporationCronJob` groups scoped characters by corporation (`groupCharacterIdsByCorporation` in `src/supabase.js`) and sends one queue message per corp, avoiding the same-corp concurrent-reconcile race described above. **Phase 4 of the cron → Workflows migration is complete** (`docs/cron-to-workflows/04-per-corporation.md`): the scheduled path moved to a per-corporation fan-out Vercel Workflow — the cron route `start()`s a like-named workflow that groups the characters by corp itself (the shared `enumerateCorporations` step in `src/workflows/lib.ts`, returning the same one-group-per-corp set plus singleton groups for unresolved characters) and runs one `'use step'` per corp across a couple of statically-assigned lanes. A corp's characters stay together in one step (never split — that would recreate the concurrent-reconcile race), and only *different* corps run concurrently; each failing corp is caught-and-continued with a summary rethrown so failures surface in Observability. Per-corp token refresh, role-fallback and heartbeats stay inside the untouched job module (`forEachCorporation`). All three migrated, all `source: 'vercel-workflow'`: `corp-wallet-transactions` (`corpWalletTransactionsWorkflow`, the safe keyed-append leader), `corp-industry-jobs` (`corpIndustryJobsWorkflow`, the first SCD-2 reconciler), and `corp-assets` (`corpAssetsWorkflow`, the biggest-volume paginated reconciler and original race victim — last). `fanOutPerCorporationCronJob` is now unused on the scheduled path (deleted in phase 5). Only the scheduled trigger moves; the on-demand "Refresh ESI" queue path is untouched until phase 5.
...
| `character_mercenary_den_share` | Opt-in den sharing: one row = "this character's owner shares their dens **and** enemy-den intel with this alliance." Audience widened from corporation to alliance (existing corp shares were migrated to that corp's alliance). `mercenary_den_shared_with_caller(reg_id)` matches a share row against `my_alliance_ids()` and drives the RLS policies on `character_mercenary_den_over_time` and `mercenary_den_enemy_intel`; both helpers run on **invoker** rights (no SECURITY DEFINER) — a share row naming its own audience, plus owner names coming from the world-readable `character_directory` rather than `registration`, is what removed the need for definer bridges. `character_id` is a registration uuid (the grantor). The UI writes all of a user's registrations at once. RLS: alliance members read shares aimed at their alliances, owners read/insert/delete their own | `character_id`, `alliance_id`, `created_at` |
...
| `sheet_csv` | The industry-planning spreadsheet's static CSVs (StaticInputs/StaticOutputs/invention/types, twines+miros modes), re-encoded from the `sde_*` mirror by the `sde-mirror` workflow's `encodeSheets` step (`src/jobs/sheetCsv.js`) after each SDE build. Served at `/sheets/[file]` for Sheets `=IMPORTDATA()`. Public-read RLS, service-role write | `name` (PK, e.g. `static-inputs-twines.csv`), `data` (CSV text), `sde_build`, `updated_at` |
...
- `blueprint_search(type_ids, system_ids, structure_ids, character_ids, corporation_ids, kind_filter, below_me, below_te, researchable_only, group_mode, row_limit)` — the MCP `list_blueprints` query (docs/mcp-tools-spec.md §1): unions `character_blueprint` + `corp_blueprint`, resolves each row's `location_id` to a system (via `sde_station` / `universe_structure`), applies every filter, optionally collapses identical (type, ME, TE) stacks (`group_mode = 'type'` / `'type_location'`), and returns one json object whose totals cover the whole filtered set while only `row_limit` rows are itemized. `below_me`/`below_te` are OR'd when both are given; `researchable_only` drops types with no manufacturing activity (reaction formulas, which permanently read ME/TE 0/0). Security invoker, so RLS scopes it like a direct select
...
- **SDE mirror workflow:** `sde-mirror` is the second Vercel Workflow (`src/workflows/sdeMirror.ts`) and the first with real multi-step orchestration: its cron route (`/api/cron/sde-mirror`, 12:21 UTC — a fifth cron dispatch shape that just `start()`s the workflow) kicks a run that discovers CCP's current SDE build (the `latest` zip URL 302s to an immutable build-pinned URL carrying `x-sde-build-number`) and decides whether to re-ingest via `planMirror` (`src/jobs/sdeMirror.js`, shared by the workflow's `planRun` and the CLI): it **skips** — a ~5s no-op that just closes the heartbeat (`finalizeSkipped`) — only when the build is already completed, was produced by the currently-deployed git commit (`currentCommitSha()` from `VERCEL_GIT_COMMIT_SHA`/`COMMIT_SHA`, stored in `sde_mirror_state.commit_sha` by `finalizeBuild`), **and** is less than 7 days old; a new SDE build, a new code deployment (so a transform change re-runs), or 7-day staleness each force a **full re-ingest**, writing every JSONL entry into its `sde_<stem>` table in cursor-resumable slices — each step Range-reads only its entry's compressed bytes from the zip (no full download, no unzip binary; `node:zlib` inflates it) and upserts 500-row chunks until its ~35s budget runs out, returning the line cursor the orchestrator loops back in. Files ingest concurrently across a bounded pool of lanes (`INGEST_LANES`, statically assigned largest-first so replay is deterministic and the longest slice chains start first; safe because each file owns its own table and every step reads the same immutable zip), each file's slice chain staying sequential within its lane. Tables are minted at ingest by the service-role-only `ensure_sde_mirror_table()` RPC (so a new CCP file needs no migration), rows are stamped with `sde_build` and stale ones swept per file. The tail steps start as soon as the specific files they read have landed rather than after the whole ingest: station names resolve via ESI into `sde_npc_station_name` once `sde_npc_stations` drains, the `esf_data` re-encode starts once its 7 input tables drain, the `sheet_csv` re-encode (`encodeSheets`) starts once `sde_types`+`sde_blueprints` drain, and finalize — last, after everything — refreshes `sde_blueprint_product` (`sde_refresh_views()`), marks the build completed, and closes the heartbeat the first step opened. All logic lives in `src/jobs/sdeMirror.js` (CLI-runnable with an unbounded budget). The app reads these mirror tables at runtime through the async `src/sde*.ts` loaders, and the `esf-data` cron/workflow job reads them to encode the ship-fitting protobufs into `esf_data` — the loader cutover is done, and both the `sde:build`/`src/buildSde.js` and `esf:build` build-time steps were removed, so the build touches no SDE at all.
- **MCP server:** `/api/mcp` (Streamable HTTP, via `mcp-handler`; route at `src/app/api/mcp/route.ts`) exposes read-only tools over the extracted data, registered from three modules sharing the plumbing in `lib.ts`:
  - `tools.ts` — `search_assets`, `list_clones` (location, current ship, active implants, jump clones, next-jump time), `list_blueprints`, `list_structures`, `list_industry_jobs`, `list_market_orders`, `search_transactions`, `appraise_items`, plus the two SDE-backed industry tools `blueprint_for_product` (how to build an item + its material bill) and `blueprints_using_material` (what consumes an item). The pure filter/format seams for the two list tools live in `blueprintQuery.ts` / `structureQuery.ts`, which pull in no I/O (only `ramda`/`ts-pattern`) so `pnpm test` can exercise them without a client.
  - `estateTools.ts` — `browse_assets` (hangars by place rather than by item name: every location with stack counts, or one location's contents, over the `*_asset_location_summary()`/`*_location_contents()` RPCs — a raw id drills into a container or ship, which the summary never lists), `wallet_summary` (per-character balances, corp division balances from the newest journal entry per division, and a `corp_wallet_journal` rollup by `ref_type`), `list_mercenary_dens` (state, development/anarchy, reinforcement timers).
  - `industryTools.ts` — `industry_cost_indices` (latest `industry_system_index` per system/activity, defaulting to the caller's structures ∪ watched systems, cheapest-first when one activity is named), `list_job_slots` (occupied vs free manufacturing/research/reaction slots per character; ceilings from the slot skills in `src/app/industry/jobSlots.ts`, shared with `/character` so the two can't drift), `rigs_for_blueprint` (which Upwell rigs bonus a product, optionally intersected with what's fitted to one of your structures).

  Every tool carries `annotations: { readOnlyHint: true, openWorldHint: … }` — both hints matter to the client, which buckets an unannotated tool as "other" and prompts for it, since the MCP defaults are pessimistic (`readOnlyHint` false, `openWorldHint` true). They are server-declared assertions, not enforcement; what actually constrains a tool is the RLS-scoped bearer client. `openWorldHint` is `false` on every tool but `appraise_items`, which is the only one that leaves the deployment (it prices against innomin.at); the rest read this project's Postgres and nothing else. `list_blueprints` and `list_structures` (docs/mcp-tools-spec.md §§1, 3) push their filters down rather than fetching and slicing: `list_blueprints` calls the `blueprint_search()` RPC (item, owner, `system`, `structure`, `kind`, `below_me`/`below_te`, `researchable`, `group` collapsing and `limit` all happen in SQL), and `list_structures` — the caller's monitored `corp_structure` rows with hull and class, system and security, state, fuel expiry, service split and fitted rigs — sends its `system`, `name` and `owner` as `eq`/`ilike`/`in` predicates (only the `services` filter stays in JS, since PostgREST can't substring-match inside a jsonb array and the table holds dozens of rows, not the five figures that motivated the blueprint RPC). Tools accept fuzzy names (items/systems/owners resolved via the SDE search helpers) and return resolved names plus a `data_refreshed` freshness stamp from `latest_heartbeats()`.

  `list_market_orders` and `list_industry_jobs` take an optional `as_of` (parsed by the same `parseAtParam` the Sheets `at=` uses) that reconstructs a past snapshot through the SCD-2 functions `character_orders()` / `character_industry_jobs()` / `corp_industry_jobs()` instead of the `is_current` views — no grant was needed for this even though those functions only ever `grant execute … to service_role` explicitly (they were written for the Sheets endpoints, which query as service role). Every function in `public` is already executable by `anon`/`authenticated`, both from Postgres's own default EXECUTE-to-`PUBLIC` and from the schema-wide `alter default privileges … grant all on functions to anon, authenticated, service_role` near the top of `schema.sql` — which is exactly why `ensure_sde_mirror_table`/`sde_refresh_views` have to `revoke execute … from public, anon, authenticated` to be service-role-only. What keeps the snapshot functions safe is that they're SECURITY INVOKER, so RLS on the underlying `*_over_time` tables scopes rows to the caller (an `anon` caller can reach them and gets nothing).

  The two blueprint tools read only the mirrored SDE data (`src/sdeBlueprints.ts`) — not the extract DB — and adjust the material bill through the `eve-industry` `cost()` modifiers exposed as tool params (`runs`, `material_efficiency`, `structure`, `rig`, `security`); passing a monitored `structure_id` instead derives the structure role bonus, ME rig and system-security multiplier from `corp_structure`/`corp_structure_rig` (RLS-scoped) and overrides those three params. **The rig is gated on applicability** (`src/app/blueprint/rigs.ts`, the same filter tables behind `/blueprint/[typeID]`): only a fitted ME rig whose filter covers the product's group/category can discount it, and the rigs skipped for that reason are reported. Because applicability is per product, the structure is loaded once per call and the bonus resolved per row — `blueprints_using_material` reports `rig_applied` on each row rather than naming one rig for the whole listing. `list_structures` deliberately reports **no** derived material bonus: what a fitted rig is worth depends on the product, and the arithmetic belongs to `eve-industry`'s `cost()` — so it lists the rigs and leaves the costing to the tools that know what's being built (`docs/mcp-tools-spec.md` §3 proposed an `me_bonus` field; it was implemented, then removed for this reason).
