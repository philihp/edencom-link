-- blueprint_search(), redefined so its body reads character_blueprint's
-- renamed owner column (registration_id) — the companion to the migration
-- immediately before this one, which does the actual rename.
--
-- This file deliberately contains nothing but the function and its grant.
-- test/sql/blueprint_search.sql creates stand-in tables named after the real
-- views and then `\i`-includes this file to define the function against them,
-- so anything else here (an ALTER TABLE, a DROP VIEW) would break that test.
-- That is also why the rename lives in the previous migration rather than here.
--
-- The body is otherwise unchanged and reproduced verbatim from schema.sql. Its
-- `character_ids` parameter stays: renaming it is step 7 of
-- docs/registration-id-rename.md and has to ship with every RPC call site at
-- once, so `b.registration_id = any(character_ids)` reads oddly on purpose.
...
begin
  ...
  where n.nspname = 'public' and p.proname = 'blueprint_search';

  if def is null then
    raise exception 'function public.blueprint_search() is missing';
  end if;
  if def like '%b.character_id%' then
    raise exception
      'blueprint_search() body still references b.character_id — it would fail at its next call, not here';
  end if;
  if def not like '%b.registration_id%' then
    raise exception 'blueprint_search() body does not reference b.registration_id';
  end if;
  if def not like '%character_ids%' then
    raise exception 'blueprint_search() lost its character_ids parameter';
  end if;
end $$;
